...
#show: rendercv.with(
  ...
  entries-highlights-bullet: text(fill: rgb("059669"))[•],
...
// Override link to keep icon attached to body (no line break between text and icon):
#import "@preview/fontawesome:0.6.0": fa-icon
// original-link is the Typst builtin, saved by rendercv before it shadows link
#let link(dest, body, icon: none, if-underline: none, if-color: none) = context {
  let config = rendercv-config.get()
  let show-icon = if icon != none { icon } else { config.at("links-show-external-link-icon") }
  let show-underline = if if-underline != none { if-underline } else { config.at("links-underline") }
  let show-color = if if-color != none { if-color } else { true }
  let font-size = config.at("typography-font-size-body")
  let link-color = config.at("colors-links")

  let body = [#if show-underline [#underline(body)] else [#body]]
  if show-icon {
    let ext-icon-content = fa-icon("external-link", size: 0.65em)
    let icon-width = measure(ext-icon-content).width
    let ext-icon = box(
      width: icon-width, height: 0pt, baseline: 0pt,
      place(dy: -0.75em, ext-icon-content),
    )
    body = [#body#h(font-size / 4)#ext-icon]
  }
  body = [#if show-color [#set text(fill: link-color);#body] else [#body]]
